    ...
    var vm = new Vue({
        ...
        methods: {
            ...
            login() {
                ...
                if(flag){
                    //发送登陆请求
                    this.$http.get('student/user/login?identity=' + this.identity + "&name=" + this.name).then(result => {
                        var result = result.body;
                    // 登陆成功
                    if (result.Code == 200) {
                        // 判断是否为学生 若是管理员提示去管理员页面登陆
                        if(result.Data == "0") {
                            this.popUp('登陆成功,即将跳往主页面...')
                            setTimeout(function () {
                                location.href = 'student/user/index'
                            }, 2000);
                        }else if(result.Data == "1"){
                            alert("老师登陆请在老师登陆页面进行登陆")
                        }
